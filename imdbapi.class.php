...
class IMDB {
	...
	if($this->debug) echo $strUrl."\n";  
	$oCurl = curl_init($strUrl);
	  curl_setopt_array($oCurl, array(
		  CURLOPT_VERBOSE => FALSE,
		  CURLOPT_HEADER => TRUE,
		  CURLOPT_HTTPHEADER => array(
			  'Accept-Language: '.str_replace("_", "-", $this->language).", ".array_shift(array_values(explode("_", $this->language))).';q=0.5'
		  ),
	...
	private function getMovieDetails(){
		...
		if(preg_match('~(\d{6,})~', $this->input, $result)){
			if($this->debug)echo "Doing movie details call\n";
			$this->ImdbId =  'tt'.$result[0];
			$url = $this->getAPIURL("title/tt".$result[0]."/maindetails?");
		}else{
			if($this->debug)echo "Trying to find ImdbID\n";
	...
	public function getPlot(){
		if($this->debug)echo "Doing movie plot call\n";
